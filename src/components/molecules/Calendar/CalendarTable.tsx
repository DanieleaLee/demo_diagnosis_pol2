...
const CalendarTable = ({
  ...
  // 해당 년도의 공휴일
  const getSpecificYearHolidays = hd.getHolidays(baseMonth.year());

  // 해당 년도의 공휴일의 월일
  const holidayDateArr = getSpecificYearHolidays.map((el) => el.date.split(' ')[0].slice(5));

  // 그 달의 시작하는 주
  const firstWeek = baseMonth.clone().startOf('month').week();

  // 1년 = 52주 + 몇 일
  // moment.js에서는 내년의 첫 주인 1로 표시..
  // 마지막 주가 1이면 53
  const lastWeek = baseMonth.clone().endOf('month').week() === 1 ? 53 : baseMonth.clone().endOf('month').week();
  ...
  const datesArray = useCallback(() => {
      ...
      result = result.concat(
        ...
        <tr key={week}>
          {Array(7)
            .fill(0)
            .map((data, index) => {
              let days = baseMonth.clone().startOf('year').week(week).startOf('week').add(index, 'day');

              // DAY가 해당 월의 DAY가 아닐 경우.
              if (days.format('MM') !== baseMonth.format('MM')) {
                return (
                  <td key={index}>
                    <button
                ...
                holidayDateArr.includes(
                  `${days.format('MM')}-${+days.format('D') < 10 ? `0${days.format('D')}` : days.format('D')}`
                )
              ) {
                return (
                  <td key={index}>
                    <button
                      ...
                      className={classNames({
                        selectHoliday: selectedDate === days.format('YYYY-MM-DD'),
                      })}
    ...
    return result;
  }, [baseMonth, firstWeek, lastWeek, holidayDateArr, selectedDate, selectDateHandler, theme]);

  useEffect(() => {
    //국가코드를 받아오는 api 호출 및 countrycode state 업데이트
    async function getCountryCode() {
      const res = await axios.get<GetCountryCodeApi>('https://api.ip.pe.kr/json');
