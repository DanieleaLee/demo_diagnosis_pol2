...
import { flexCenter, flexRow, flexRowBetween } from '@styles';
...
interface Props {
  ...
  title: string;
  checkedItems: string[];
  filteredData: Array<{ id: number | string; name: string }>;
  show: boolean;
    ...
    <div css={[dropdownContainerWrap({ width })]}>
      ...
      <div css={dropdownUpperBtnWrap} className={classNames({ active: show })}>
        ...
        {show === true && (
          <div css={searchContainerWrap}>
            <BiSearchAlt2 color="#B6B6B6" size={16} />
            <input
            ...
            />
            <BiCaretDown color='#2F3B43' size={16} onClick={openDropdownHandler}/>
          </div>
        )}
        {/* 체크된 체크박스가 없고 드롭다운이 닫혀있을 경우 */}
        {checkedItems.length === 0 && show === false && (
          <div css={filledArrowDownIconWrap} onClick={openDropdownHandler}>
            <BiCaretDown color='#2F3B43' size={16}/>
          </div>
        )}
      </div>
      ...
      {show && (
        <div css={dropdownLowerContentsWrap}>
          {/* 체크박스 그룹 컴포넌트 */}
          <MultipleCheckbox data={filteredData} onChangeHandler={onChangeHandler} checkedItems={checkedItems} />
...
const MultipleCheckbox = ({ data, onChangeHandler, checkedItems }: MultipleCheckboxProps) => {
  return (
