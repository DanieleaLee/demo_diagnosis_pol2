...
import {
  DataType,
  PeriodData,
} from "@lucian2Components/organisms/PfOverlayChartPerformance/index";
...
const getDateIndex = (obj_, yearMonthList)=>{
  const startDateIndex = yearMonthList.indexOf(obj_.start_date.replaceAll('-','.').slice(0,-3))
  const endDateIndex = yearMonthList.indexOf(obj_.end_date.replaceAll('-','.').slice(0,-3))
      ...
      { barList && barList.map((el,i)=>{
          return (
            ...
            <div
              ...
              onClick={() =>{ onSelectDatebar(el.id)}}
            >
                <PfOverlayBalloon className="tooltip" triangleSize={5} width={105}>
...
export default PfOverlayDateBar;
