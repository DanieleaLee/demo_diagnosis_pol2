...
const columns = useMemo(() => {
    return [
        {
            accessor:"Keyword",
            Header:({column}) => (
                <div css={[TbHeaderCss, css`text-align:left;`]}>
                    <Typography.Body4 color={Colors.buttonSubmit}>Keyword</Typography.Body4>
                </div>
            ),
            Cell:({row}) => (
                <div css={[TbCellCss, css`text-align:left;`]}>{row.original.keyword}</div>
            ),
            width:260
        },
        {
            ...
            Header:({column}) => (
                <div css={[TbHeaderCss]}>
                    <TbHeaderSort size={7.65} column={column} columnName={"Trending"}/>
                </div>
            ),
            Cell:({row}) => (
                <div css={[TbCellCss, css`${flexCenter};`]}>
                    <SingleScoreChart isLoading={false} animation={true} value={row.original.trending} fontSize={10} lineWidth={1.5} containerCss={css`height: 24px;width: 24px;`}/>
                </div>
            ),
            width:80,
        },
        {
            ...
            Header:({column}) => (
                <div css={[TbHeaderCss]}>
                    <TbHeaderSort size={7.65} column={column} columnName={"Ratio"}/>
                </div>
            ),
            Cell:({row}) => (
                <div css={[TbCellCss]}>{row.original.ratio}</div>
            ),
            width:130
        },
        {
            ...
            Header:({column}) => (
                <div css={[TbHeaderCss]}>
                    <Typography.Body4 color={Colors.buttonSubmit}>Power</Typography.Body4>
                </div>
            ),
            Cell:({row}) => {
              return (
                <div>
                   <NewTechRangeBar rowData={DATA} rowId={row.original.id}/>
                </div>
              )
            },
            width:75
        },
        {
            ...
            Cell:({row}) => {
                return (
                    <button onClick={() => onRemove(row.original.id)} css={[closeIconWrapCss]}>
  ...
  const {setGlobalFilter,getTableProps,getTableBodyProps,headerGroups,rows,prepareRow,...tableProps} = useTable({ columns, data }, useGlobalFilter, useSortBy);
