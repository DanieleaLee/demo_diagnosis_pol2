...
import CloseIcon from "@lucianComponents/atoms/Icon/CloseIcon";
import RangeBarArrow from "@lucianComponents/atoms/Icon/RangeBarArrow";
import RangeBarBtn from "@lucianComponents/atoms/RangebarBtn";
import { css, useTheme, Theme } from "@emotion/react";
import { flexCenter, flexColumn, flexRow } from "src/styles";
import * as Typography from "@styles/typography";
import Colors from "src/styles/colors";
import * as Balloon from '../Balloon/index';
import Buttonable from "src/components/atoms/Buttonable";import { CgClose } from 'react-icons/cg';
import { BiBluetooth, BiEdit } from 'react-icons/bi';
...
const NewTechRangeBox = ({
  ...
  return (
    <div css={[css`position:absolute; z-index:999;`]} ref={nodeRef}>
      {/* Apply 버튼 클릭 시 해당 값이 apply되는 컴포넌트 */}
      {open && (
        <Balloon.Box width={contentsWidth ? contentsWidth : 197} height={142.56} arrowPosition={"top"}
        arrowAlign={"center"}>
          <div css={[rangeBarContentsWrap({contentsWidth: contentsWidth,theme})]}>
             <div css={rangeBarInnerWrap}>
                <div css={closeIconWrapStyle} onClick={closeHandle}>
                   <CgClose color="#DBDADA" />
                </div>
                <Typography.Base fontSize="10px" fontWeight="600" lineHeight="12px" color={Colors.rangeTitleText} css={rangeTextStyle}>
                    Range
                </Typography.Base>
                ...
                <div css={[rangeSliderContainerWrap({theme})]}>
                    ...
                    // Thumb 의 zindex 조정하기 위한 것
                     className={classNames('bar bar--zindex-3', {'bar--zindex-5': minValue > max - 50})}/>
                </div>
                <div css={rangeSliderInputStyle}>
                    <input
                    type="range"
                ...
                <div css={sliderStyle}>
                    ...
                    {pressedMin && (
                    <div css={[sliderRangeStyle({ theme })]} style={{width: `${Math.min(minLimit, maxValue)}%`,left: 0}}/>)}
                    {pressedMax && (
                    <div css={[sliderRangeStyle({ theme })]} style={{width: `${100 - Math.max(maxLimit, minValue) - minLimit}%`,left: `${Math.max(maxLimit, minValue)}%`}}/>)}
                    {!pressedMin && !pressedMax && (
                    <div css={[sliderRangeStyle({ theme })]} style={{width: `${(maxValue - minValue)}%`,
                   left: `${minValue}%`}}/>)}
                    </div>
                </div>
...
export default NewTechRangeBox;
