...
import ColumnMilestone from "@alvinComponents/molecules/ColumnMilestone";
import RightMenu from "@alvinComponents/molecules/RightMenu";
import TechTree from "@alvinComponents/molecules/TechTree";
import MpTitle from "@alvinComponents//molecules/MpTitle";
import {
  totalStepsList,
  tagsDummy,
  memoContent,
  UniverseInfo,
} from "@alvinComponents/Dummy";
import Button from "src/components/_temp/alvin/atoms/Buttons";
...
import CustomTable, {
  TColumn,
  TRow,
} from "@taehyunComponents/molecules/CustomTable";
import Image from "next/image";
import Filter from "@lucianComponents/organisms/Filter";
...
const Template_304 = (props) => {
  ...
  useEffect(() => {
    ...
    const colData: TColumn[] = titles.map((title: string) => {
      const sortable =
        title !== "Tags" && title !== "Performance" ? true : false;
      let header = title;
    ...
    const fetchData = async (perPage) => {
      try {
        const result = await axios(
          `http://10.10.1.4:4444/api/v1/temp/iu/list?cnt=${perPage}&page=0`
        );
        console.log(result);
        if (result.status === 200) {
              ...
              actions={[
                {
                  icon: (
                    <Image
                      width={23}
                      height={23}
                      src={`/img/taehyun/bx-trash-alt.png`}
                      alt="bx-trash-alt"
                    />
                  ),
                  tooltip: "action1",
