...
import { BiAnalyse, BiChart, BiCustomize, BiGlobe, BiTargetLock } from "react-icons/bi";
...
const OverlayConfigSidebar = ({ clickedLayer, onClickLayerHandler, data }: Props) => {
  return (
    <div css={[bodyContainerCss]}>
      <div css={[bodyInnerWrapCss]}>
        <div css={[bodyUpperWrap]}>
          <div css={[contentsWrap]}>
            {data.slice(0,-1).map((j,v) => <SidebarSubTitle key={j.id} item={j} icon={iconList[v].icon} onClickLayerHandler={onClickLayerHandler} clickedLayer={clickedLayer} />)}
          </div>
        </div>
        <div css={[customRowWrap]}>
          <div css={[flexRowStyle]}> 
            <div css={[iconWrap]}>{iconList[iconList.length - 1].icon}</div>
            <div css={[customWrap]}><Typography.Body2 color={Colors.selectCategoryAmountBg}>{data[data.length -1]?.title}</Typography.Body2></div>
          </div>
          <TextButton.Tiny onClick={() => {}} title="Build Layer" color={Colors.backgroundWhite} css={[btnCss]}/>
        </div>
      </div>
    </div>
  );
...
export default OverlayConfigSidebar;
